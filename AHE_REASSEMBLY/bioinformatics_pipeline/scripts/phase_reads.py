...
def get_files(args):
	...
	if args.vcf:
		...
		vcf = os.path.join(args.dir, 'variants', '%s.qual_filtered%s.cov_filtered%s.vcf.gz' % (args.lineage, args.qual, args.dp))
...
def phase(args, files, genome, vcf, dir):
	...
	bam = '-I ' + ' -I '.join(files)
	subprocess.call("/usr/lib/jvm/java-8-openjdk-amd64/jre/bin/java -Xmx%sg -jar %s -T ReadBackedPhasing -R %s %s --variant %s -o %s" %
...
def get_haplo(outdir, vcf, seq, lineage, args):
	f = gzip.open(vcf, 'r')
	...
	suboutdir = os.path.join(outdir, '%s_qual%s_cov%s' % (lineage, args.qual, args.dp))
...
def main():
	...
	if args.haplo:
		seq = get_seq(genome)
		get_haplo(outdir, out_vcf, seq, args.lineage, args)
