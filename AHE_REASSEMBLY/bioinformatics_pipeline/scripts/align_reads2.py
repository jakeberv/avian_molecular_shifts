...
def get_qual(args, files, genome, dir):
	...
	# makes the raw VCFs, only outputting variant SNPs
        subprocess.call("/usr/lib/jvm/java-8-openjdk-amd64/jre/bin/java -Xmx%sg -jar %s -T UnifiedGenotyper -R %s %s -o %s "
...
def recalibrate(args, files, genome, vcf, dir):
	for file in files:
		...
		subprocess.call("/usr/lib/jvm/java-8-openjdk-amd64/jre/bin/java -Xmx%sg -jar %s -T BaseRecalibrator -R %s -knownSites %s -I %s -o %s" %
			        (args.mem, args.gatk, genome, vcf, file, recal), shell=True)
		# print the recal reads
		subprocess.call("/usr/lib/jvm/java-8-openjdk-amd64/jre/bin/java -Xmx%sg -jar %s -T PrintReads -R %s -I %s --BQSR %s -o %s" %
