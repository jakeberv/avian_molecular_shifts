...
def get_args():
	...
	parser.add_argument(
                ...
                type=str,
                default=None,
                help='Full path to flash binary.'
                )
...
def run_pear(args, info, outfiles1, dir):
	...
	pear_out =  ['%s.notCombined_1.fastq' % out_stem,
		     '%s.notCombined_2.fastq' % out_stem,
	             '%s.extendedFrags.fastq' % out_stem]

	outfiles2 = ['%s_R1_paired_2.fq.gz' % out_stem,
                     '%s_R2_paired_2.fq.gz' % out_stem,
                     '%s_R1_assembled_2.fq.gz' % out_stem]

	subprocess.call("%s %s %s -o %s -d %s -t %s" % (args.flash, outfiles1[0], 
                                                        outfiles1[2], args.sample, dir, args.CPU), shell=True)
