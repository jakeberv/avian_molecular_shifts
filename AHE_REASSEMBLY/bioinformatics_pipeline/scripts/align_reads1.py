...
def align_seq(args, r, seq, dir):
	...
	subprocess.call("/usr/lib/jvm/java-8-openjdk-amd64/jre/bin/java -Xmx%sg -jar %s -T RealignerTargetCreator -R %s -I %s -o %s -nt %s" % 
                        (args.mem, args.gatk, seq, out6, intervals, args.CPU), shell=True)
	# indel realigner
	subprocess.call("/usr/lib/jvm/java-8-openjdk-amd64/jre/bin/java -Xmx%sg -jar %s -T IndelRealigner -R %s -I %s -targetIntervals %s -o %s" % 
			(args.mem, args.gatk, seq, out6, intervals, out7), shell=True)
	

	# remove the files
	[os.remove(x) for x in [out1a, out1as, out1b, out2a, out2b, out3a, 
                                out3b, out4a, out4b, out5a, out5b, out6, intervals]]
