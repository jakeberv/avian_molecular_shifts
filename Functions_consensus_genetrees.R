...
##code to test the alignments generated by alisim, generated by grimoire GPT4
#requires, ape, parallel, pbmapply
{
  # Function to process a single fasta file
  process_fasta_file <- function(file_path) {
    sequences <- read.dna(file_path, format = "fasta")
    
    # Capture the printed output of the sequences
    temp <- capture.output(print(sequences))
    
    # Extract the line with base frequencies
    base_freq_line <- temp[16]
    
    # Use regular expression to extract the numerical values (base frequencies)
    base_freqs <- as.numeric(unlist(regmatches(base_freq_line, gregexpr("[0-9.]+", base_freq_line))))
    
    # Check if the base frequencies are correctly parsed
    if (length(base_freqs) != 4) {
      stop("Failed to parse the base frequencies correctly.")
    }
    
    # Assign names to the base frequency vector
    names(base_freqs) <- c("A", "C", "G", "T")
    
    # Calculate GC content
    gc_content <- sum(base_freqs[names(base_freqs) %in% c("G", "C")])
    
    return(c(base_freqs, GC_Content = gc_content))
  }
  
  # Function to process directory with parallel processing and analyze GC content
  process_fasta_directory <- function(directory_path) {
    fasta_pattern <- "\\.fa$|\\.fas$|\\.fasta$"
    fasta_files <- list.files(directory_path, pattern = fasta_pattern, full.names = TRUE)
    ...
    # Using pbmclapply for parallel processing
    results_list <- pbmclapply(fasta_files, process_fasta_file, mc.cores = no_cores)
    
    # Convert results to a data frame with numeric values
    results_df <- do.call(rbind, results_list)
    results_df <- data.frame(File = basename(fasta_files), results_df, stringsAsFactors = FALSE)
    
    # Extract data type from file names and add it as a new column
    results_df$DataType <- sub("^(\\w+)_.*", "\\1", results_df$File)
    
    # Group by DataType and calculate min, max, and average GC content
    gc_stats <- results_df %>%
      group_by(DataType) %>%
      summarise(Min_GC = min(GC_Content),
                Max_GC = max(GC_Content),
                Avg_GC = mean(GC_Content))
    
    return(list(Results = results_df, GC_Stats = gc_stats))
  }
